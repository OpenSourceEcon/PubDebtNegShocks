...
@jit
def sim_timepath(args, H_ind=None, k_ind=None, x1_ind=None, S_ind=None,
                 zt_vec=None, rand_seed=None):
    (n_vec, c_min, K_min, x1, x2, Hbar, beta, gamma, alpha, delta, mu,
        rho, sigma, A_min, z_min, yrs_in_per, T, z0, k20) = args
    if H_ind is None:
        H_ind = 0
    if k_ind is None:
        k_ind = 0
    ...
    while (t_ind < T - 1) and not GameOver:
        ...
        k2t = k2t_vec[t_ind]
        zt = zt_vec[t_ind]
        k2tp1_args = (n_vec, c_min, K_min, Hbar, beta, gamma, alpha,
                      delta, mu, rho, sigma, A_min, yrs_in_per)
        (k2tp1, c1t, Ht, c2t, wt, rt, rbart, rbart_an, GameOver) = \
            get_k2tp1(k2t, zt, k2tp1_args)
        ...
        if GameOver:
            GameOver_vec[t_ind:] = GameOver
        t_ind += 1

    return (H_ind, k_ind, x1_ind, S_ind, zt_vec, GameOver_vec, c1t_vec,
