...
par.debt = 20/100; % 5/100 for Figure 7, 20/100 for Figure 8
...
disp(index);
    ...
    for c_nd=1:size(at,2) %Calculate path of K according to Eq. of Motion.
            wage_nodebt(c_nd,index) = at(1,c_nd,index) * (1-par.b);
            ...
            mpk_nodebt(c_nd,index) = at(1,c_nd,index) * (par.b);
...
%Transfer is a certain percentage of average savings in a no-transfer world
transfer_amount=par.debt*par.kss;
...
clear c_nd
    ...
    for c_d=1:size(at,2)
        ...
        wage_debt(c_d,index) = at(1,c_d,index) * (1-par.b);
        ...
        mpk_debt(c_d,index) = at(1,c_d,index).*par.b;
...
%Generate variables needed for graphing Aggregate Utility
col=0;
count=1;
for i=gridrf
    col=col+1;
...
% Figure 7 or 8 (please set line 22 appropriately).
figure
...
clear ax;
title({['Mean Utility, Linear production with aggregate risk']; [' $\mu=$',num2str(par.mu(1)), ' $\sigma=$',num2str(par.sigma), ' $D=$',num2str(par.debt*100),'$\%$ of EK']})
xlabel('$Er^f$')
ylabel('Er')    
